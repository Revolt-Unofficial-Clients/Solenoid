...
const Settings: Component = () => {
    ...
    const updateStatus = (
        ...
        if (mode && status) {
            ...
            client.api.patch('/users/@me', {
                status: {
                    presence: Solenoid.settings.status || client.user?.presence,
                    text: Solenoid.settings.statusText || client.user?.status,
                },
            });
        }
    };
    ...
    function logoutFromRevolt() {
        ...
        if (client.session) client.destroy();
    }
    ...
    return (
        <div
            ...
            <div
                ...
                <div>
                    <div>
                        <h3>Logged In as {client.user?.username}</h3>
                    </div>
                    ...
                    <div>
                        <img
                            src={
                                client.user?.avatar
                                    ? `${client.config?.features.autumn.url}/avatars/${client.user?.avatar?.id}`
                                    : `https://api.revolt.chat/users/${client.user?.id}/default_avatar`
                            }
                ...
                <div class='bg-base-200 m-3 p-3 rounded-lg'>
                    <form
                        ...
                        onSubmit={async (e) => {
                            console.log('Clicked');
                            e.preventDefault();
                            const file = await client.uploadAttachment(
                                `solenoid-avatar-${client.user?.id}`,
                        ...
                        <div class='' id='2'>
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    Solenoid.setNickname(e.currentTarget.value)
                                }
                            />
                        </div>
                        <div class='item' id='3'>
                                ...
                                <img
                                    ...
                                    src={
                                        Solenoid.avatarImage()
                                            ...
                                            : member_avatar_url() || client.user?.avatar
                                            ? `https://autumn.revolt.chat/avatars/${
                                                  Solenoid.servers.current_server.me
                                                      ?.avatar?.id ||
                                                  client.user?.avatar?.id
                                              }`
                                            : `https://api.revolt.chat/users/${client.user?.id}/default_avatar`
                                    }
                                    width={64}
            ...
            <div
                            ...
                            <button
                                onClick={() => {
                                    Solenoid.setStatusList(
                                        Solenoid.statuslist().filter(
                                            (obj) => obj.id !== prefab.id
                                        )
                                    );
                                }}
                                class='btn btn-error'>
                ...
                <div class='flex gap-2'>
                    ...
                    <input
                        class='input'
                        onChange={(e: any) =>
                            Solenoid.setNewStatus(e.currentTarget.value)
                        }
