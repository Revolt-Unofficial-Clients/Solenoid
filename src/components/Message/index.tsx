...
const Message: Component<MessageComponent> = ({client, message, settings, setter, signal, colour, deleteFunction}) => {
    return (
        ...
        <div class="solenoid-message-author">
            ...
            {settings.suffix && (
                <span>{settings.showSuffix ? " says " : ":"}</span>
            )}
            ...
            <div class="button-container">
                {
                    message.author?._id === client.user?._id && !editing() && (
                        <>
                            <div role="button" class="edit" onClick={() => {
                                setEditing(true)
                                setEditMessageId(message._id);
                                setNewMessage(message.content ?? "");
                            }}><span>Edit</span></div>
                            <div role="button" class="delete" onClick={() => deleteFunction(message)}><span>Delete</span></div>
                        </>
                    )
                }
                <div
        ...
        <div class="content">
            {editing() && editMessageId() === message._id ? (
                <div>
                    ...
                    />
                    <div role="button" class="done" onClick={() => {
                        message.edit({
                            content: newMessage()
                        }).then(() => {
                            setEditing(false)
