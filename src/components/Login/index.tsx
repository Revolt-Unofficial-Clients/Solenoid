import { Component, createSignal, Setter, Accessor, batch, onMount } from "solid-js";
                    ...
                    {window.location.hostname !== "localhost" && (
                        <form
                            onSubmit={async (e) => {
                                ...
                                if (email() && password()) {
                                    try {
                                        await loginWithEmail(email() ?? "", password() ?? "").catch((e) => {
                                            throw e
                                        });
                                    } catch (e) {
                        ...
                        >
                            <div>
                                ...
                                {error() && <span class="solenoid-error">An error has occurred while logging in: {error()}</span>}
                            </div>
                        </form>
                    )}
                    {solenoid_config.session && <button id="existingsession" onClick={() => loginWithSession(solenoid_config.session)}>Use Existing Session</button>}
                </div>
            )}
            </>
    )
